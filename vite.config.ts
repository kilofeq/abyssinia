...
function staticRoutes(): Plugin {
  const install = (server: ViteDevServer | PreviewServer) => {
    const redirects = new Map(readFileSync(resolve(server.config.root, 'public/_redirects'), 'utf8')
    ...
    server.middlewares.use((req, res, next) => {
      ...
      if (path !== '/' && path !== '/en/' &&
        ...
        res.end(req.method === 'HEAD' ? undefined : readFileSync(resolve(server.config.root, 'public/404.html')))
        return
      }
      next()
    })
  }
  return { name: 'abyssinia-static-routes', configureServer: install, configurePreviewServer: install }
}
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      async handler(template, context) {
        ...
        const { render } = await context.server.ssrLoadModule('/src/entry-server.tsx')
    ...
    handleHotUpdate({ file, server, modules }) {
      ...
      if (file.includes('/src/') && /\.(tsx?|json)$/.test(file) && !file.endsWith('/client.ts')) {
